...
// Measure the live text box rather than assuming desktop columns and lines.
function splitMessageIntoPages(text) {
  const box = message();
  const style = getComputedStyle(box);
  const width = Math.max(16, box.clientWidth - parseFloat(style.paddingLeft) - parseFloat(style.paddingRight) - 20);
  const height = box.clientHeight - parseFloat(style.paddingTop) - parseFloat(style.paddingBottom);
  const maxLines = Math.max(1, Math.floor(height / parseFloat(style.lineHeight)));
...
function inventoryMenu() {
  showText(Game.inventory.length ? `もちもの\n${Game.inventory.join("\n")}` : "もちものは、まだありません。");
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("keydown", (event) => {
    if (!Game.started || event.target.closest("button")) return;
